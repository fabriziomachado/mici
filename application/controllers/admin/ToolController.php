 ...
 * Begin Document
 */
class ToolController extends MI_Controller
{
    /**
     * Construct
     */
	...
	$this->load->model('codeigniter/tool_model');
    }
	...
	if ($this->input->server('REQUEST_METHOD') === 'POST')
	{
	    ...
	    {
		foreach ($this->input->post('tools') as $tool_id)
		{
		    $post_data = $this->tool_model->delete($tool_id);
	...
	$data['tools'] = $this->tool_model->get();
	...
	$this->load->view('admin/tool/index', $data);
	...
	if ($this->input->server('REQUEST_METHOD') === 'POST')
	{
	    ...
	    $post_data = $this->tool_model->add($clean_post);
	    if(is_array($post_data))
	    {
		$this->session->set_flashdata('error_message', $post_data['error']);
		$this->session->set_flashdata('error_code', $post_data['code']);
	...
	else
	{
	    ...
	    $this->load->view('admin/partials/header', $data);
	    $this->load->view('admin/tool/detail', $data);
	    $this->load->view('admin/partials/footer', $data);
	}
    }
	...
	if ($this->input->server('REQUEST_METHOD') === 'POST')
	{
	    ...
	    $post_data = $this->tool_model->update($this->uri_array['id'], $clean_post);
	    if(is_array($post_data))
	    {
		$this->session->set_flashdata('error_message', $post_data['error']);
		$this->session->set_flashdata('error_code', $post_data['code']);
	...
	else
	{
	    ...
	    $data['tool'] = $this->tool_model->get($data['id']);
            $data['all_tags'] = $this->tag_model->get_all_tags('json');
	    $data['existing_tags'] = $this->tool_model->my_tags('json');
...
/* End of file ToolController.php */
/* Location: ./application/controllers/admin/ToolController.php */